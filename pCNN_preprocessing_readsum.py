...
#データ読み込みを改良➡高速化
#unityへデータを送るのも実装
#前処理を自作で実装➡レイヤー数3,stride=0.1で0.9突破←間違い
#脳波データsumで読み込む➡0.9突破（交差検証なし）➡プログラム上の合計値ではない
#プログラム上の合計値でやると過学習
...
data_x = np.zeros((all_sample_number,extraction_freq),dtype="float64")
data_y = np.zeros(all_sample_number,dtype="int32")
print("data_x.shape",data_x.shape)
tmp = np.zeros((ch_number,extraction_freq))
        ...
        for k in a:
            ...
            if label_jdg[0][delete_label] != 1:
                ...
                for ch in range(ch_number):
                    tmp[ch] = data_run1["X"][head:tail,ch]
                #print(np.sum(tmp,axis=0))
                data_x[run_cnt+label] = np.sum(tmp,axis=0)
                #print(data_x[run_cnt+label])
            ...
            if label_jdg[1][delete_label] != 1:
                ...
                for ch in range(ch_number):
                    tmp[ch] = data_run2["X"][head:tail,ch]
                data_x[run_cnt+label] = np.sum(tmp,axis=0)
                #print(data_x[run_cnt+label])
            ...
            if label_jdg[2][delete_label] != 1:
                ...
                for ch in range(ch_number):
                    tmp[ch] = data_run2["X"][head:tail,ch]
                #data_x[run_cnt+label] = sum(data_run3["X"][head:tail,0:3])
                data_x[run_cnt+label] = np.sum(tmp,axis=0)
                #print(data_x[run_cnt+label])
...
for asn in range(all_sample_number):
    avg = np.average(data_x[asn])
    std = np.std(data_x[asn])
    data_x[asn] = (data_x[asn] - avg) / std
"""
#"""
#列
data_x = data_x.reshape(extraction_freq,all_sample_number)
for freq in range(extraction_freq):
    avg = np.average(data_x[freq])
    std = np.std(data_x[freq])
    data_x[freq] = (data_x[freq] - avg) / std
data_x = data_x.reshape(all_sample_number,extraction_freq)
#"""
#全体
...
for i in range(3):
    ...
    y = data_x[random_label[i]] #ランダムに表示
    #y = data_x[0][i+3742]
    plt.plot(x,y)
...
fft_wave_complex = np.zeros((all_sample_number,extraction_freq),dtype="complex128")
fft_wave_abs     = np.zeros((all_sample_number,extraction_freq),dtype="float64")
...
fft_fre = fftfreq(n=data_x[0].size, d=1/sampling_freq)
...
#--------------------------------input_dataの型を計算で求める
input_data  = np.zeros((all_sample_number,65,65),dtype="complex128")

f, t, input_data[:] = scipy.signal.stft(data_x[:],fs=sampling_freq,nperseg=nperseg_number,noverlap=over_lap_number)
...
input_data = input_data.reshape(all_sample_number,len(f),len(t),1)
...
filter_number.append(20) #24
filter_number.append(40) #48
filter_number.append(80) #96
