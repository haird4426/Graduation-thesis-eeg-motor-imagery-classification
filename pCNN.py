...
for i in range(all_sample_number):
    ...
    data_ch1[i] = signal.lfilter(filter,1,data_ch1[i])
    data_ch2[i] = signal.lfilter(filter,1,data_ch2[i])
    data_ch3[i] = signal.lfilter(filter,1,data_ch3[i])
#"""
...
for i in range(all_sample_number):
    ...
    filter = signal.firwin(numtaps=tap_number,nyq=nyquist_freq,cutoff=0.5)
...
for i in range(all_sample_number):
    ...
    filter = signal.firwin(numtaps=tap_number,nyq=nyquist_freq,cutoff=[2,60])
...
for i in range(all_sample_number):
    f, t, input_data[0][i] = scipy.signal.stft(data_ch1[i],fs=sampling_freq,nperseg=nperseg_number,noverlap=over_lap_number)
    f, t, input_data[1][i] = scipy.signal.stft(data_ch2[i],fs=sampling_freq,nperseg=nperseg_number,noverlap=over_lap_number)
    f, t, input_data[2][i] = scipy.signal.stft(data_ch3[i],fs=sampling_freq,nperseg=nperseg_number,noverlap=over_lap_number)
...
#data_label = data_label.reshape(each_data_number*3*subject_number)
#print(data_label.shape) #(each_data_number,)
#print(input_data.shape)
...
dropout_rate = 0.1
model = Sequential()
first_filter_number  = 24
second_filter_number = 48
third_filter_number  = 96
fourth_filter_number = 192

first_kernel_size  = 12
