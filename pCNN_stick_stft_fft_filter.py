...
import os
from get_nearest_value import getNearestValue
...
for i in range(subject_number): #各被験者ごとのループ
    ...
    read_brain_data(sub,data_x,data_y,stride,data_magnification,delete_label,extraction_freq,run_cnt)
"""
    #毎回各被験者のデータを読み込む
...
random_label = []
for i in range(3):
    random_label.append(random.randint(0,all_sample_number-1))
...
#fft_pass = np.where(np.abs(fft_fre)!=50,fft_fre,0)
#fft_pass = np.where(np.abs(fft_fre)!=51,fft_fre,0)
...
#--------------------------------input_dataの型を計算で求める
input_data  = np.zeros((ch_number,all_sample_number,height,width),dtype="complex128")
input_data_stick = np.zeros((all_sample_number,height*3,width),dtype="float64")
...
input_data = np.real(input_data)

#stft描画
#amp
plt.pcolormesh(t,f,input_data[0][0],vmin=0)
#plt.pcolormesh(t,f_stick,input_data_stick[0],vmin=0)
plt.ylim([f[1],f[-1]])
...
#input_data = input_data.reshape((all_sample_number,ch_number,height,width))
...
for label in range(all_sample_number):
    for i in range(height*3):
        for j in range(width):
            if input_data_stick[label][i][j] == 0:
                print("input_label:",label)
                print("i:",i)
                print("j:",j)
                break
...
input_data_stick = input_data_stick.reshape((all_sample_number,len(f)*3,len(t),1))
...
####データの分割####
x_train, x_test, y_train, y_test = model_selection.train_test_split(input_data_stick, data_y, test_size=test_rate)
